...
float DelayReady
FormList Property StoneSpells Auto
...
; 				debug.Trace("STANDINGSTONE: it's been a day since the spell was last added")
				...
				RemoveSpellsFromList(akActionRef as actor)
...
Function RemoveSpellsFromList(Actor player)

	int index = StoneSpells.GetSize()

	While index

        index -= 1
        Spell buff = StoneSpells.GetAt(index) as Spell
