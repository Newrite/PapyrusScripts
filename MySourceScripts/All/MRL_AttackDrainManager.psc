...
Function BowWeaponCalc(Weapon Weap)
    ...
    ;Рассчет значения расхода ресурса
    DrawnValue = GetDrainValueRanger((GetEquippedWeaponWeight(Weap) + GetAmmowWeightValue()) * GetNormalAttackMult())
...
Function CrossBowWeaponCalc(Weapon Weap)
    ...
    if Target.HasPerk(HotReloadPerk)
        ReloadValue = GetDrainValueRanger((GetEquippedWeaponWeight(Weap) + GetAmmowWeightValue()) * GetNormalAttackMult()) * 0.7
    else
        ReloadValue = GetDrainValueRanger((GetEquippedWeaponWeight(Weap) + GetAmmowWeightValue())* GetNormalAttackMult())
    endif
...
float Function GetEquippedWeaponWeight(Weapon Weap)
    ...
    if Weap != none
        ...
        If Weap.GetWeight() <= 0.0 && Weap.GetBaseDamage() > 0
            WeaponWeight = Weap.GetBaseDamage() as Float / 5.0
        elseif Weap.GetWeight() <= 0.0
            WeaponWeight = Weap.GetCritDamage()
        else
...
Event OnAnimationEvent(ObjectReference akSource, string asEventName)
    ...
    if asEventName == "JumpUp"
        ...
        ;Влияние веса надетой экипировки на стоимость прыжка, больше ничего его стоимость не трогает
...
; Перестраховка что бы дрейны не залипли
...
; Также вне стейтов в апдейте не должно быть ничего
Event OnUpdate()
EndEvent
