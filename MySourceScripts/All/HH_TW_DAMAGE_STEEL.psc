Scriptname HH_TW_DAMAGE_STEEL extends activemagiceffect  
...
float Base = 45 ; базовый урон кинжала
...
;Debug.Notification("Буст к шансу" +HH_Global_chance.GetValue() as int)
endif
if akTarget.HasKeyword(ActorTypeGhost)|| akTarget.HasKeyword(ActorTypeUndead)
Dmg=Dmg*0.1
endIf
float penetr= 0.04 + HH_GL_Perk_penetr.GetValue()+((K2-K1)*0.005) ; пробитие 
if penetr>0.8
penetr = 0.8
endIf
...
int random = Utility.RandomInt(0, 99)
	if (random < HH_GL_Perk_Find.GetValue())&&!akTarget.IsPlayerTeammate()&&(!akTarget.HasKeyword(ActorTypeDwarven))
