Scriptname HH_DAMAGE_SILVER extends activemagiceffect  
...
float Base = 55 ; базовый урон кинжала 
...
Float crit = (HH_Global_Heavy_crit.GetValue()) ; 2.5 например 
...
Float crit = (HH_Global_Heavy_crit.GetValue()+(skill2*0.01)) ; 2.5 например 
Int whole = crit as Int ; 2 
Float decimal = (crit - whole) * 10 ; 
Debug.Notification("Критический урон x" + whole + "." + decimal as Int ) 
endif 
If (PlayerRef.IsDetectedBy(akTarget))&&Lrandom<fakechance && HH_Global_Ligt_crit.GetValue()>1
...
Elseif (PlayerRef.IsDetectedBy(akTarget))&& Lrandom>fakechance&& HH_Global_Ligt_crit.GetValue()>1
