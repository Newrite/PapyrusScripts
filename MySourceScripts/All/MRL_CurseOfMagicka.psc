...
function UpdateGlobals()
	...
	if EnchantingSkillAdvance >= 100.0
		float damage = MaxCurseDamage
		if CursedActor.WornHasKeyword(HetotTome)
			damage = 75.0
		endif
		gCurseDamage.SetValue(damage)
		gCurseCost.SetValue(MaxCurseCost)
		gCurseSummon.SetValue(MaxCurseSummon)
	else
		float damage = EnchantingSkillAdvance * 0.5
		if CursedActor.WornHasKeyword(HetotTome)
			damage = 75.0
		endif
		gCurseDamage.SetValue(damage)
...
Function EnchantmentHetot()

	CursedActor.ModActorValue(sCrusedValue, 10.0)
...
Event OnSpellCast(Form akSpell)
	...
	if CursedActor.IsInCombat()
		...
		bool isHetot = CursedActor.WornHasKeyword(HetotTome)
