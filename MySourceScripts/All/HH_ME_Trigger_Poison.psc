...
Event OnEffectStart(Actor akTarget, Actor akCaster)
	...
	speedMultDebuff = SpeedMultDebuffDisplay.GetValue() * 1.0
	Staks = HH_Glob_Poison_Stacs.GetValue() as int
	DMG=DMG*(1-(TargetPoisonResist/100.0))
	DMG_B=DMG_B*(1-(TargetPoisonResist/100.0))
	speedMultDebuff = speedMultDebuff * (1-(TargetPoisonResist/100.0))
	HH_Spell_damage_Poison.SetNthEffectMagnitude(0, DMG)
	HH_Spell_damage_Poison.SetNthEffectMagnitude(1, SpeedMultDebuffDisplay.GetValue())
	HH_Spell_damage_Poison.SetNthEffectMagnitude(2, DMG_B)
