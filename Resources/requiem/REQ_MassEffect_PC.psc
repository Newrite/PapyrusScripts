...
Event ScriptShutDown(string eventName, string strArg, float numArg, Form sender)
	...
	UnregisterForUpdate()
	REQ_AutoUpdater UpdateControl = GetOwningQuest().GetAlias(0) as REQ_AutoUpdater
	if (UpdateControl.VersionInstalled.GetValue() >= 18200.0)
		;these values were controlled externally before 1.8.2
...
Event OnMenuClose(String MenuName)
	;Debug.Trace("Skills Menu closing, time for full evaluation! mass = "+Player.GetAV("mass")+" effmass = "+Player.GetAV("infamy"))
	Update_Ratios()
	;Debug.Trace("Results: " + ratios_armor + " " + ratios_weapon)
	GoToState("full_evaluation_start")
	Full_Evaluation()
	;Debug.Trace("Skills Menu closed, mass = "+Player.GetAV("mass")+" effmass = "+Player.GetAV("infamy"))
EndEvent
